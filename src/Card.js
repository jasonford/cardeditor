        ...
        triggerUpload((file)=> {
          ...
          ref.put(file, metadata).then(function () {
            ...
            ref.getDownloadURL().then((downloadURL) => {
              ...
              db.ref(that.props.deckName+'/'+that[".key"]).update(update);
            });
          });
        });
      }
      db.ref(this.deckName+'/'+this[".key"]).update(update);
    };
  }
  safeMarkup (field) {
    ...
    //  render katex fields
    markup = replaceFunction(markup, 'inlinekatex', function (input) {
    ...
    markup = replaceFunction(markup, 'image', function (input) {
      return '<div style="width:100%;height:100%;position:absolute;left:0;top:0;background-image:url('+input+');background-size:contain;background-repeat:no-repeat;background-position: center center;"></div>';
    });
    //  sanitize as final step
    markup = sanitize(markup);

    //  render emphasized parts
    markup = replaceFunction(markup, 'em', function (input) {
      return '<b><i>' + input + '</i></b>';
    });
  ...
  remove () {
    ...
    return function () {
      if (that.state.tryingToRemove) {
        setTimeout(() => {
          db.ref(that.props.deckName+'/'+that.props[".key"]).remove();
        });
      }
